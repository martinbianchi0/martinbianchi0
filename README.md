...
- **[Probabilistic robotics from scratch](https://github.com/martinbianchi0/probabilistic-robotics-from-scratch)** — motion models, a discrete Bayes filter, MCL, both EKF Jacobians, Rao-Blackwellized FastSLAM and six path planners compared. The individual build-up to the maze robot above, with **per-stage provenance separating the course's scaffolding from my own code**.
- **[Classical computer vision](https://github.com/martinbianchi0/Vision-Learning)** — three projects. *Segmentation and counting*: Otsu, morphology, connected components, and splitting touching objects via local maxima of the distance transform. *Panoramas*: SIFT + ANMS and Lowe matching, with **the homography written from scratch** — DLT via SVD plus a hand-rolled RANSAC, not `cv2.findHomography` — and OpenCV supplying only the detector and the warping, not the estimation. *Stereo 3D reconstruction.* Segmentation is individual; panoramas in a pair.
- **[ML from scratch](https://github.com/martinbianchi0/Machine-Learning)** — four individual assignments, and **no scikit-learn because the assignment forbade ML libraries**: regression (linear, Ridge and Lasso by hand), classification on an imbalanced and a multiclass problem (logistic, LDA, random forest), **a full MLP with backpropagation, ADAM and early stopping — validated against an equivalent PyTorch implementation**, and unsupervised methods (K-means, GMM, DBSCAN, PCA by hand) plus a VAE.
- **[mlflow casero](https://github.com/martinbianchi0/mlflow-casero)** — a PostgreSQL schema where ML reproducibility is enforced by database constraints rather than documentation.
